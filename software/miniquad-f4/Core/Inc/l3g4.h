...
#define WHO_AM_I_VALID 			((uint8_t)0xD3)
...
typedef struct {
	L3G4_Config_t config;
	SPI_HandleTypeDef *spi;
	GPIO_TypeDef* csPort;
	uint16_t csPin;
	volatile uint16_t drdyPin;
	volatile uint32_t dataAvai;
...
void L3G4_InstallGPIO(L3G4_t *l3g4, GPIO_TypeDef* csPort, uint16_t csPin, uint16_t drdyPin);
